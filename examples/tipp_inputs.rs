...
pub fn make_inputs_mapper(
    params: (&ModelInputs<InputTy>, &TiPpInputs, ChessBucketsMirrored, impl OutputBuckets<ChessBoard>),
    wdl: impl WdlScheduler,
) -> ModelInputsMapper<ChessBoard> {
    let tipp = params.1.clone();

    ModelInputsMapper::build(
        params.0,
        move |pos, step, (((((stm_tipp, ntm_tipp), stm_psqt), ntm_psqt), bucket), target)| {
            let mut cnt = 0;
            ...
            tipp.map_features(
                pos,
                |stm| {
                    stm_tipp[stm_cnt] = stm.try_into().unwrap();
                    stm_cnt += 1;
                },
                |ntm| {
                    ntm_tipp[ntm_cnt] = ntm.try_into().unwrap();
                    ntm_cnt += 1;
                },
            );
            ...
            target[1 + usize::from(pos.result)] = 1.0;
        },
    )
}

#[derive(Clone)]
pub struct TiPpInputs {
    threats: Arc<Threats>,
    masks: [u64; 64],
}
...
impl Threats {
    ...
    fn map_single(&self, piece: usize, src: usize, dest: usize, target: usize) -> Option<usize> {
        if piece == Piece::PAWN {
            ...
            /* || (dest > src && target % 6 == Piece::PAWN - 2) */
            {
                return None;
            }
