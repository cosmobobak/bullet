...
fn main() {
    ...
    // trainer.load_from_checkpoint("checkpoints/xxx-400");
...
fn build_network(num_inputs: usize, max_active: usize, output_buckets: usize, hl: usize) -> (Graph, Node, Node) {
    ...
    let l1 = builder.new_affine("l1", hl, output_buckets);
    ...
    let ntm_subnet = l0.forward(nstm).screlu();
    let out = stm_subnet.concat(ntm_subnet);
    let out = l1.forward(out).select(buckets).sigmoid();
