...
fn main() {
    ...
    let (mut graph, output_node) = build_network(inputs.size(), hl, 8, true);
...
fn build_network(inputs: usize, hl: usize, output_buckets: usize, no_wdl: bool) -> (Graph, Node) {
    ...
    let targets = builder.create_input("targets", Shape::new(heads, 1));
    ...
    let l3w = builder.create_weights("l3w", Shape::new(heads * output_buckets, 32));
    let l3b = builder.create_weights("l3b", Shape::new(heads * output_buckets, 1));
    ...
    if no_wdl {
        ...
        operations::softmax_crossentropy_loss(builder, predicted, targets);
    }
